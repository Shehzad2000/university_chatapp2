...
class SemesterData extends StatefulWidget {
  const SemesterData({Key? key}) : super(key: key);

  @override
  State<SemesterData> createState() => _SemesterDataState();
}
...
class _SemesterDataState extends State<SemesterData> {
 SemesterBloc? semesterBloc;
  ...
  Widget build(BuildContext context) {
    ...
    children: [
      Container(
        ...
        child: Center(child: text("Here you got Semester list:-",FontWeight.normal,18.0,Colors.black38)),
      ),
      Divider(
        indent:20,
        endIndent: 20,
                    ...
                    child:Slidable(
                        ...
                        children: [
                          ...
                          SlidableAction(
                            onPressed: (BuildContext context) async {
                              Navigator.of(context).push(MaterialPageRoute(builder: (context)=> Semester(requestModel: SemesterRequestModel(semesterID:state.list[index].semesterID!,semesterStatus:state.list[index].semesterStatus ,semesterName:state.list[index].semesterName  ),)));
                            },
                      ...
                      endActionPane:  ActionPane(
                        ...
                        children: [
                          SlidableAction(
                            onPressed: (BuildContext context) async {
                              DeletePressed(state.list[index].semesterID);
                            },
                      ...
                      child: Padding(
                        padding:  EdgeInsets.only(top:MediaQuery.of(context).padding.top+2.0,bottom:MediaQuery.of(context).padding.bottom+2.0),
                        child: ListTile(
                          leading:const CircleAvatar(
                              child:Icon(CupertinoIcons.heart)
                          ),
                          title: text(state.list[index].semesterName.toString(),FontWeight.normal,20.0,Colors.blue),
  ...
  void DeletePressed(String? semesterID){
      semesterBloc!.add(
          DeleteSemesterEvent(
            semesterID
          )
 ...
 Future OpenDialog(SemesterRequestModel model)=>showDialog(context: context, builder: (context)=>AlertDialog(
     shape: RoundedRectangleBorder(
     ...
     content:SizedBox(
       ...
       child: SingleChildScrollView(
         child: Column(
           children: [
            const Center(
               child: Text("Semester Detail",style: TextStyle(color: Colors.black),),
             ),
            const Divider(
             ...
             Row(
               children: [
                 ...
                 Container(
                     margin:  EdgeInsets.only(top:MediaQuery.of(context).size.height*0.1/13,left: MediaQuery.of(context).size.height*0.1/13),
                     child: Text('${model.semesterName}',style:const TextStyle(color: Colors.blue,fontWeight: FontWeight.normal,fontSize: 18),)),
             ...
             Row(
               children: [
                 ...
                 Container(
                     margin:  EdgeInsets.only(top:MediaQuery.of(context).size.height*0.1/13,left: MediaQuery.of(context).size.height*0.1/13),
                     child: Text('${model.semesterStatus}',style:const TextStyle(color: Colors.blue,fontWeight: FontWeight.normal,fontSize: 18),)),
